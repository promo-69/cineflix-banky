...
export const AuthService = {
	...
	async registerUser(document_id: string, email: string, phone: string, password_raw: string) {
		...
		const user = await User.createOne({ document_id, email, phone, account_number, password_hash });
		const token = jwt.sign({ id: user.id, document_id: user.document_id }, JWT_SECRET, { expiresIn: '1h' });
		return { user, token };
	},
