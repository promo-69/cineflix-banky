...
export const handle: Handle = async ({ event, resolve }) => {
	...
	// Excluimos /api/external/, /api/admin/, /api/v1/pos/, /api/transactions/ y /api/profile/
	if (
		path.startsWith('/api/') &&
	...
	// 1.8 Proteger /api/transactions y /api/profile con cookie de sesión común
	if (path.startsWith('/api/transactions') || path.startsWith('/api/profile')) {
