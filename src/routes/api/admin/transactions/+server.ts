...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const { source_user_id, type, amount, destination_document, destination_account, destination_phone, bank_code } = body;
		...
		if (type === 'transfer') {
			...
			resultTx = await LedgerService.processTransfer(source_user_id, destination_document, destination_account, amount, reference);
			...
			resultTx = await LedgerService.processMobilePayment(source_user_id, destination_document, destination_phone, bank_code, amount, reference);
		} else {
