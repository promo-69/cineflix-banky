<script lang="ts">
  ...
  let canSubmitPm = $derived(isPmDocValid && isPmPhoneValid && pmBankCode !== '' && isPmAmountValid && pmAmountStr !== '');
    ...
    {#if currentTab === 'transferencia'}
      <form method="POST" action="?/transfer" use:enhance={() => { 
        ...
        {#if transferEntryMode === 'contact'}
          <div class="field">
            ...
            {#if data.accounts.length === 0}
              <div style="padding: 12px; background: var(--surface-2); border-radius: var(--r-sm); font-size: 13px; color: var(--muted);">
        ...
        {#if transferEntryMode === 'manual'}
          ...
          <div class="field">
            ...
            <div class="input-wrap">
              <Input type="text" restrict="numeric" id="destination_account" name="destination_account" class="mono" bind:value={destinationAccount} placeholder="01020000000000000000" maxlength={20} isValid={destinationAccount ? isAccountValid : undefined} />
            </div>
          </div>
        {/if}
        ...
        <div class="field">
          ...
          <Input type="text" restrict="numeric" id="amount" name="amount" class="amount mono" bind:value={amountStr} placeholder="0.00" isValid={amountStr && !isAmountValid ? false : undefined} style="padding-left: 48px;">
            {#snippet icon()}
        ...
        <div style="display: flex; justify-content: flex-end;">
          <Button type="button" variant="primary" disabled={!canSubmit} {loading} style="width: 100%; border: none;" onclick={() => confirmAction(() => submitTransferBtn.click())}>
            Transferir
          </Button>
      ...
      <form method="POST" action="?/mobilePayment" use:enhance={() => { 
        ...
        {#if pmEntryMode === 'manual'}
          ...
          <div class="field">
            ...
            <Input type="text" restrict="phone" id="destination_phone" name="destination_phone" class="mono" bind:value={pmPhone} placeholder="04141234567" maxlength={11} isValid={pmPhone ? isPmPhoneValid : undefined} />
          </div>
        ...
        <div class="field">
          ...
          <Input type="text" restrict="numeric" id="amount_pm" name="amount" class="amount mono" bind:value={pmAmountStr} placeholder="0.00" isValid={pmAmountStr && !isPmAmountValid ? false : undefined} style="padding-left: 48px;">
            {#snippet icon()}
              <span style="font-weight: 500;">Bs.</span>
            {/snippet}
          </Input>
        ...
        <div class="spacer" style="height: 24px;"></div>
        <div style="display: flex; justify-content: flex-end;">
          <Button type="button" variant="primary" disabled={!canSubmitPm} loading={pmLoading} style="width: 100%; border: none;" onclick={() => confirmAction(() => submitPmBtn.click())}>
            Pagar
          </Button>
